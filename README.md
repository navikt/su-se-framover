...
- Vi bruker **ikke** `autoLogin`. Uinnloggede kall mot `/api` gir `401`, og frontend redirecter da selv til `/oauth2/login` (se [src/api/apiClient.ts]()). Ved utlogging går brukeren til `/oauth2/logout` (global logout hos Entra), lander på appen uten session, og blir sendt videre til `/oauth2/login` på nytt av samme 401-håndtering.
