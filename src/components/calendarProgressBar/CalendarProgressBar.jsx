...
export const CalendarProgressBar = ({ startPeriode, datoIDag, sluttPeriode }) => {
    ...
    function generateBarStyle() {
        ...
        let background =
            'linear-gradient(to right, lightgreen 0%, lightgreen ' + greenWidth + '%, lightblue ' + greenWidth + '%)';
    ...
    const showMonths = () => {
        ...
        return (
            <div className="progress">
                {someArr.map(i => (
                    <span key={i.text} className={i.class} style={i.css}>
                        {i.text}
                    </span>
                ))}
            </div>
    ...
    return (
        <div style={{ backgroundColor: 'lightgrey' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', margin: '1em' }}>
                <p>
                    Stønad innvilget: {startPeriode.getDate()} {startPeriode.getMonth() + 1}{' '}
                    {startPeriode.getFullYear()}
                </p>
                <p>
                    Dato i dag: {datoIDag.getDate()} {datoIDag.getMonth() + 1} {datoIDag.getFullYear()}
                </p>
                <p>
                    Stønad slutt: {sluttPeriode.getDate()} {sluttPeriode.getMonth() + 1} {sluttPeriode.getFullYear()}
                </p>
            </div>
            <div style={generateBarStyle()}>{showMonths()}</div>
        </div>
    );
};
