...
describe('Tests related to displaying data from a filled application', () => {
    ...
    test('Tests that clicking a radio button in the Flyktning group, results in the right values', () => {
        const x = render(<Soknad />);
        const personOpplysningerRadioButtons = x.getAllByRole("radio")

        const flyktningRadioButtons = []
        personOpplysningerRadioButtons.forEach( obj => {
            if(obj.name === "flyktning")
                flyktningRadioButtons.push(obj)
        })

        //klikker ja
        fireEvent.click(flyktningRadioButtons[0])
        expect(flyktningRadioButtons[0].checked).toBe(true)
        expect(flyktningRadioButtons[1].checked).toBe(false)

        //klikker nei
        fireEvent.click(flyktningRadioButtons[1])
        expect(flyktningRadioButtons[0].checked).toBe(false)
        expect(flyktningRadioButtons[1].checked).toBe(true)
