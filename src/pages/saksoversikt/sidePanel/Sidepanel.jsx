...
const Sidepanel = () => {
    ...
    function displayCorrectChevron() {
        if (displaySideContent) {
            return (
                <div
                    style={{ height: '40px', width: '40px', display: 'flex', justifyContent: 'center' }}
            ...
            return (
                <div
                    style={{ height: '40px', width: '60px', display: 'flex', justifyContent: 'center' }}
                    onClick={() => handleDisplaySideContentClick()}
                >
    ...
    const DisplayHendelser = () => {
        return aktivStønadsperiode.hendelser.map(hendelse => (
            <span key={hendelse}>
                <p>{hendelse}</p>
                <br />
            </span>
    ...
    return (
        <ExpandableDiv className={classNames(displaySideContent ? 'open' : 'closing')}>
            <div
                style={{
                    display: 'flex',
