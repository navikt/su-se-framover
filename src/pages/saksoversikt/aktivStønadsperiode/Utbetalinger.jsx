...
import {utbetalingUpdated, hendelseUpdated} from "../../../redux/saksoversikt/saksoversiktActions";
    ...
    const Utbetaling = ({item, index}) => {
        ...
        return (
            <div style={{display: "grid", gridTemplateColumns: '1fr 1fr 1fr 1fr', marginBottom: '1em'}}>
                ...
                <p>{item.beløp}</p>
                {
                    item.status === "stoppet" ? <p style={{color:'red', fontWeight: 'bold'}}>{item.status}</p>
                        : item.status === "Utbetalt" ? <p style={{color:'green', fontWeight: 'bold'}}>{item.status}</p>
                        : <p >{item.status}</p>
                }
                <p>
                ...
                </p>
                {
                    ...
                    >
                        <div style={{padding:'2rem 2.5rem'}}>
                            ...
                            <p>{reverseDateString(item.datoForUtbetaling)} {item.beløp} {item.status}</p>
                            ...
                            <RadioGruppe legend={"Velg status for utbetaling"}>
                                {options.map(({ label, value }) => (
                                    <Radio
                                        ...
                                        onChange={e =>{handleActions(e, index)
